...
/**
 * Trim a raw FPP playlist to what the cloud import needs: name, the
 * description, and all three running-order sections. SET:IQ uses the
 ...
 * leadIn/leadOut ride along because the cloud folds all three sections
 * into the playlist's song list.
 */
function setiq_playlist_forward($name, $pl) {
    $desc = '';
    ...
    $sec = function ($k) use ($pl) {
        return isset($pl[$k]) && is_array($pl[$k]) ? $pl[$k] : [];
    };
    return [
        'name'         => $name,
        'desc'         => $desc,
        'leadIn'       => $sec('leadIn'),
        'mainPlaylist' => $sec('mainPlaylist'),
        'leadOut'      => $sec('leadOut'),
    ];
}

/**
 * Read every playlist that already exists on this FPP box.
 *
 * Reads the playlist JSON files straight off the disk — FPP keeps one
 * `<name>.json` per playlist in the media directory, and this plugin
 * runs ON the box, so there's no reason to round-trip through the HTTP
 * API where names with spaces/punctuation can get mangled in the URL
 * (an unmatched name can come back 200 with no mainPlaylist, which
 * pushed as an "empty" playlist). Falls back to the API only when the
 * directory can't be read. Returns raw FPP playlist objects — the
 * cloud normalizes the shapes.
 */
function setiq_local_playlists() {
    global $settings;
    if (isset($settings['playlistDirectory']) && $settings['playlistDirectory']) {
        $dir = $settings['playlistDirectory'];
    } elseif (isset($settings['mediaDirectory']) && $settings['mediaDirectory']) {
    ...
    if (is_array($files) && count($files) > 0) {
        ...
        foreach ($files as $file) {
            $pl = json_decode((string) @file_get_contents($file), true);
            if (!is_array($pl)) continue;
            $name = (isset($pl['name']) && is_string($pl['name']) && trim($pl['name']) !== '')
                  ? trim($pl['name']) : basename($file, '.json');
            $out[] = setiq_playlist_forward($name, $pl);
        }
        if (count($out) > 0) return $out;
    }
    // Fallback: ask fppd over HTTP (non-standard media location).
    ...
    foreach ($list as $entry) {
        ...
        if (!is_array($pl)) continue;
        $out[] = setiq_playlist_forward(
            (isset($pl['name']) && is_string($pl['name']) && $pl['name'] !== '') ? $pl['name'] : $name,
            $pl
        );
    }
    return $out;
...
function setiq_push_import($base, $key) {
    ...
    // Count what we're actually sending — if the box's own playlist files
    // have no entries, say so up front instead of letting the push look
    // healthy and the SET:IQ calendar come up empty.
    $sent = 0;
    foreach ($playlists as $p) {
        $sent += count($p['leadIn'] ?? []) + count($p['mainPlaylist'] ?? [])
               + count($p['leadOut'] ?? []);
    }
    if ($sent === 0 && count($playlists) > 0) {
        return [false, 'this box\'s ' . count($playlists) . ' playlist(s) contain'
            . ' 0 songs, so there is nothing SET:IQ could place on the calendar.'
            . ' Check FPP → Content Setup → Playlists — each playlist should'
            . ' list its sequences'];
    }
    $payload = json_encode([
        'key'       => $key,
    ...
    if (is_array($receipt) && isset($receipt['playlists'])) {
        ...
        $msg = "$pl playlist(s)"
             . ($songs === null ? " ($sent song" . ($sent === 1 ? '' : 's') . ' sent)'
                                : " with $songs song" . ($songs === 1 ? '' : 's'))
    ...
    return [true, count($playlists) . " playlist(s) ($sent song"
        . ($sent === 1 ? '' : 's') . ' sent) and ' . count($schedule)
